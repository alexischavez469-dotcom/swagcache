...
export async function getLiveProps(): Promise<PropsResult> {
  ...
  const sports: Sport[] = ['NFL', 'NCAAF', 'NBA', 'MLB', 'Soccer', 'Tennis']
  ...
  if (props.length > 0) {
    const enriched = await enrichPropsWithMatchups(props.slice(0, 180))
    return { props: enriched, live: true }
  }
