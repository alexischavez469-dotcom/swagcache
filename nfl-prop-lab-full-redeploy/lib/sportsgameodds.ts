...
function eventToProps(event: Json): Prop[] {
  ...
  for (const group of groups.values()) {
    ...
    const matchupAnalysis = `Live FanDuel line: ${fanDuelLine}. ${nBooks ? `SportsGameOdds fair market line: ${marketLine}.` : 'Fair market comparison unavailable.'}`

    const opponentName = teamName(oppObj, oppFallback)
    const role = `Live ${propType} prop for ${name} vs ${opponentName}.`

    const riskFactors: string[] = []
    if (Math.abs(delta) < 0.5) riskFactors.push('Very small gap between FanDuel and the fair market line')
