...
function enrichOne(prop: Prop, context: MatchupContext): Prop {
  const baseRisks = prop.riskFactors.filter((r) => {
    const x = r.toLowerCase()
    return !x.includes('matchup enrichment') &&
      !x.includes('role/usage summary is contextual') &&
      !x.includes('recent form and injuries') &&
      !x.includes('fair market comparison unavailable') &&
      !x.includes('rating falls back to fanduel')
  })
  ...
  if (context.signal === 'Unavailable') {
    return {
      ...
      matchupAnalysis: context.description,
      riskFactors: [...baseRisks, 'Opponent matchup data unavailable for this prop'],
    }
  }
  ...
  const riskFactors = [...baseRisks]
  if (conflict) riskFactors.push('Opponent matchup conflicts with the market lean')
  if (context.signal === 'Neutral') riskFactors.push('Opponent matchup is neutral and adds limited confirmation')
