...
function eventToProps(event: Json): Prop[] {
  ...
  for (const group of groups.values()) {
    ...
    const matchupAnalysis = `${marketSource.charAt(0).toUpperCase() + marketSource.slice(1)} is ${marketLine}, ${Math.abs(delta).toFixed(1)} ${sideWord} the FanDuel line of ${fanDuelLine}. This is a market-based lean only; matchup and role data have not been added yet.`

    const role = `${propType} projection for ${gameTime}. Live FanDuel line confirmed in the SportsGameOdds feed${nBooks ? ' and compared with the SportsGameOdds fair market line' : ''}.`
