    ...
    <article className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      <div className="flex items-center gap-2 border-t border-border px-3.5 py-2.5">
        <div className="flex items-baseline gap-1.5">
          ...
          <span className="text-xs text-muted-foreground">FanDuel</span>
        </div>
        <div
          className={cn(
      ...
      {open && (
        <div className="space-y-4 border-t border-border bg-background/40 p-3.5">
          <Section icon={BarChart3} title="Market comparison">
          ...
          {prop.recentStats && prop.recentStats.length > 0 && (
            <Section icon={BarChart3} title="Recent">
              <div className="grid grid-cols-5 gap-2">
                {prop.recentStats.slice(0, 5).map((item, idx) => (
                  <div key={`${item.label}-${idx}`} className="rounded-lg border border-border bg-secondary/40 p-2 text-center">
                    <div className="font-mono text-xs font-semibold">{item.value}</div>
                    <div className="mt-1 truncate text-[10px] text-muted-foreground">{item.label}</div>
                  </div>
                ))}
              </div>
              {prop.recentSource && (
                <p className="mt-2 text-[10px] text-muted-foreground">Source: {prop.recentSource}</p>
              )}
            </Section>
          )}
