...
function usageSummary(prop: Prop, row: Json, position?: string): { summary: string; metric: string; scoreDelta: number } {
  ...
  return {
    summary: `${prop.player}'s current ${position ?? 'NFL'} role: ${bits.length ? bits.join(' · ') : 'usage fields were limited in the current NFLMeta response'}. ${impact}`,
...
export async function enrichPropsWithNFLMeta(props: Prop[]): Promise<Prop[]> {
  ...
  const enriched = await Promise.all(top.map(async (prop) => {
    ...
    const riskFactors = prop.riskFactors.filter((x) => !x.toLowerCase().includes('role/usage summary'))
    ...
    return {
      ...
      usageSource: 'NFLMeta',
      analysisStage: 'Context enriched' as const,
      riskFactors,
    }
  }))
