...
async function fetchOpponentContext(prop: Prop): Promise<MatchupContext> {
  ...
  try {
    ...
    const weakness =
      signal === 'Favorable'
        ? family === 'rush'
          ? `${prop.opponentAbbr} has been vulnerable on the ground`
          : family === 'pass'
            ? `${prop.opponentAbbr} has been vulnerable through the air`
            : `${prop.opponentAbbr} has allowed an above-average scoring environment`
        : signal === 'Tough'
          ? family === 'rush'
            ? `${prop.opponentAbbr} has been strong against the run`
            : family === 'pass'
              ? `${prop.opponentAbbr} has been strong against the pass`
              : `${prop.opponentAbbr} has limited opposing scoring`
          : `${prop.opponentAbbr} grades close to neutral for this prop type`
