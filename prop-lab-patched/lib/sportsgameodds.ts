...
const SPORT_TO_LEAGUE: Record<Sport, string> = {
  NFL: 'NFL',
  NCAAF: 'NCAAF',
  NBA: 'NBA',
  MLB: 'MLB',
...
const BOOKMAKER_FILTER = ['prizepicks', ...COMPARISON_BOOKS].join(',')
...
function statLabel(statID: string, marketName?: string): string {
  ...
  if (marketName) {
    const cleaned = marketName
      .replace(/\bOver\/Under\b/gi, '')
      .replace(/\bMore\/Less\b/gi, '')
      .trim()
    if (cleaned.length <= 36) return cleaned
  }
  return titleize(statID)
}

function bookAbbr(id: string): string {
  return BOOK_ABBR[id] ?? id.slice(0, 3).toUpperCase()
}
...
function eventToProps(event: Json): Prop[] {
  ...
  for (const raw of Object.values(odds)) {
    ...
    if (!statEntityID || !players[statEntityID]) continue
  ...
  for (const group of groups.values()) {
    ...
    // This app is a PrizePicks board, so never fabricate a PrizePicks line from
    // fairOverUnder/bookOverUnder. Require a currently available PrizePicks entry.
    const ppOver = getBookEntry(group.over, 'prizepicks')
    const ppUnder = getBookEntry(group.under, 'prizepicks')
    const pp = ppOver ?? ppUnder
    const prizePicksLine = num(pp?.overUnder)
    if (prizePicksLine == null) continue

    const player = players[group.statEntityID] as Json
    const name = playerName(player)
    if (!name) continue

    const playerTeamID = String(player.teamID ?? '')
    const isHome = playerTeamID === homeID
    const isAway = playerTeamID === awayID
    ...
    const sportsbookLines: number[] = []

    for (const bookId of COMPARISON_BOOKS) {
      const entry = getBookEntry(group.over, bookId) ?? getBookEntry(group.under, bookId)
      const line = num(entry?.overUnder)
      if (line == null) continue
      sportsbookLines.push(line)
      consensus.push({ book: bookAbbr(bookId), line })
    }

    const fairLine = num(base.fairOverUnder)
    const marketLine = fairLine ?? median(sportsbookLines)
    if (marketLine == null) continue

    const delta = marketLine - prizePicksLine
    let recommendation: Prop['recommendation'] = delta >= 0 ? 'More' : 'Less'

    // When PrizePicks and the fair line are the same, use the de-vigged fair
    ...
    // Confidence reflects the PP-vs-market gap, with fair-price probability as
    ...
    const scale = Math.max(1, Math.abs(prizePicksLine))
    const relativeGap = Math.abs(delta) / scale
    const confidence = Math.max(1, Math.min(10, Math.round(3 + relativeGap * 35 + probabilityEdge * 10)))
    ...
    const marketSource = fairLine != null ? 'sportsbook fair line' : 'sportsbook median'

    const matchupAnalysis = `${marketSource.charAt(0).toUpperCase() + marketSource.slice(1)} is ${marketLine}, ${Math.abs(delta).toFixed(1)} ${sideWord} the PrizePicks line of ${prizePicksLine}. This is a market-based lean only; matchup and role data have not been added yet.`

    const role = `${propType} projection for ${gameTime}. Live PrizePicks line confirmed in the SportsGameOdds feed${nBooks ? ` and compared with ${nBooks} sportsbook${nBooks === 1 ? '' : 's'}` : ''}.`

    const riskFactors: string[] = []
    if (Math.abs(delta) < 0.5) riskFactors.push('Very small gap between PrizePicks and the market')
    if (nBooks < 2) riskFactors.push('Limited sportsbook comparison coverage')
    if (fairLine == null) riskFactors.push('Fair line unavailable; using sportsbook median')
    if (confidence <= 4) riskFactors.push('Market edge is thin')
    if (!riskFactors.length) riskFactors.push('Market-only model; matchup, usage, and injury context not included yet')
...
async function fetchLeague(sport: Sport, key: string): Promise<Prop[]> {
  ...
  return data.flatMap(eventToProps).sort((a, b) => b.confidence - a.confidence).slice(0, 24)
}
...
export async function getLiveProps(): Promise<PropsResult> {
  ...
  const sports: Sport[] = ['NFL', 'NCAAF', 'NBA', 'MLB', 'NHL', 'Soccer']
  const settled = await Promise.allSettled(sports.map((sport) => fetchLeague(sport, key)))

  const props = settled.flatMap((result) => (result.status === 'fulfilled' ? result.value : []))
  props.sort((a, b) => b.confidence - a.confidence)

  if (props.length > 0) {
    return { props: props.slice(0, 100), live: true }
  }
  ...
  return {
    ...
    error: errors.length
      ...
      : 'The API responded successfully, but no currently available PrizePicks props were found for the configured leagues.',
  }
}
